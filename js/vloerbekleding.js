...
function getCards() {
  ...
  titlebarTitle.textContent = data[lang][0][2][1][0][0];
    for (let i = 0; i < data[lang][0][2][1][0][1].length; i++) {
        ...
        img_MDfUj.src = `images/${data.nl[0][2][1][0][1][i][0].replace(/ /g, '-').toLowerCase()}.jpg` || 'images/logo.svg';
        img_MDfUj.setAttribute(`alt`, data[lang][0][2][1][0][1][i][0]);
        ...
        a_FZoyA.href = `${data[lang][0][2][1][0][1][i][0].replace(/ /g, '-').toLowerCase()}.html`;
        h2_qunjJ.appendChild(a_FZoyA);
        a_FZoyA.textContent += data[lang][0][2][1][0][1][i][0];
        const p_Wzyfz = document.createElement('p');
        div_HFbUm.appendChild(p_Wzyfz);
        p_Wzyfz.textContent = (data[lang][0][2][1][0][1][i][1] !== undefined) ? `${data[lang][0][2][1][0][1][i][1][0].substring(0, 200)} ... ` : `...`;
        ...
        a_piRos.href = `${data[lang][0][2][1][0][1][i][0].replace(/ /g, '-').toLowerCase()}.html`;;
